...
class sanitize
{
	 ...
	 */
	public static function accents(&$str, $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 * @return bool True.
	 */
	public static function attribute_value(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 * @return bool True.
	 */
	public static function control_characters(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 */
	public static function country(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 * @return string String.
	 */
	public static function csv(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
				static::csv($str[$k]);
	 ...
	 * @return string Email.
	 */
	public static function email(&$str = null, $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
				static::email($str[$k]);
	 ...
	 */
	public static function file_extension(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 * @return string HTML.
	 */
	public static function html(&$str = null, $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
				static::html($str[$k]);
	 ...
	 */
	public static function hostname(&$domain, $www = false, $unicode = false, $constringent = false)
	{
	 ...
	 * @return bool True.
	 */
	public static function iri_value(&$str = '', $protocols = null, $domains = null, $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 * @return string String.
	 */
	public static function js(&$str = '', $quote = "'", $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 * @return string Name.
	 */
	public static function name(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
				static::name($str[$k]);
	 ...
	 */
	public static function password(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 * @return string String.
	 */
	public static function printable(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 */
	public static function province(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
			...
			if (!isset($provinces[$str])) {
				if (false === ($str = data::array_isearch($str, $provinces, true))) {
					$str = '';
				}
			}
		}
	 ...
	 * @return string String.
	 */
	public static function quotes(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
				static::quotes($str[$k]);
	 ...
	 * @return string State.
	 */
	public static function state(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
				static::state($str[$k]);
			...
			if (!isset($states[$str])) {
				if (false === ($str = data::array_isearch($str, $states, true))) {
					$str = '';
				}
			}
		}
	 ...
	 * @return string State.
	 */
	public static function au_state(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
			...
			static::whitespace($str, 0, true);
			$str = strtoupper($str);
            $states_au = constants::STATES_AU;

			if (!isset($states_au[$str])) {
				if (false === ($str = data::array_isearch($str, $states_au, true))) {
					$str = '';
				}
			}
		}
	 ...
	 * @return string URL.
	 */
	public static function url(&$str = '', $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
				static::url($str[$k]);
	...
	public static function utf8(&$str = '')
	{
		if (is_array($str)) {
			...
			if (null === static::$_mb) {
				static::$_mb = (function_exists('mb_check_encoding') &&
			...
			if (
				...
				for ($x = 0; $x < $length; ++$x) {
					...
					elseif (($c1 & "\xc0") === "\x80") {
                        ...
                        $win1252_chars = constants::WIN1252_CHARS;
	 ...
	 * @return string String.
	 */
	public static function whitespace(&$str = '', $newlines = 0, $constringent = false)
	{
		if (is_array($str)) {
			foreach ($str as $k => $v) {
	 ...
	 * @return string String.
	 */
	public static function whitespace_multiline(&$str = '', $newlines = 1, $constringent = false)
	{
