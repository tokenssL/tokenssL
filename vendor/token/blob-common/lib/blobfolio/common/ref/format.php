...
class format
{
	 ...
	 */
	public static function fraction(&$num, $precision = 0.0001)
	{
		if (is_array($num)) {
			foreach ($num as $k => $v) {
	...
	public static function json_decode(&$str = '')
	{
		...
		for ($x = 0; $x <= $length; ++$x) {
			...
			// A comma or the end.
			if ( ($x === $length) || ((',' === $chunk[$x]) && 'slice' === $last['type'])) {
			...
			// A new quote.
			elseif ((('"' === $chunk[$x]) || ("'" === $chunk[$x])) && ('string' !== $last['type'])) {
				$slices[] = array(
					'type' => 'string',
					'from' => $x,
					'delimiter' => $chunk[$x],
	 ...
	 * @return bool True.
	 */
	public static function links(&$str, $args = null, $pass = 1, $constringent = false)
	{
	 ...
	 * @return bool True.
	 */
	public static function list_to_array(&$list, $args = null, $constringent = false)
	{
		$out = array();
		...
		$cast_types = constants::CAST_TYPES;
		if (
			('array' === $args['cast']) ||
			!isset($$cast_types[$args['cast']])
		) {
			$args['cast'] = 'string';
		}
